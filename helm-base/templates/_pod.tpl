...
spec:
  ...
  - name: {{ .name | default $root.Values.name }}
    ...
    ports:
      {{- if hasKey . "ports" }}
      {{- include "helm-base.containerExtraPorts" .ports | indent 4 -}}
      {{- end }}
      {{- if and (hasKey . "servicePort") .servicePort.enabled }}
      {{- include "helm-base.containerServicePort" $root | indent 4 -}}
      {{- end }}
    {{- end }}
    ...
    resources: 
      {{- include "helm-base.resources" $root | indent 6 -}}
    {{- end }}
    {{- if hasKey . "command" }}
    command: 
    {{- range .command }}
    ...
    securityContext:
      {{- include "helm-base.securityContext" .securityContext | indent 2 -}}
    {{- end }}
  ...
  volumes:
    {{- include "helm-base.volumes" $root | indent 2 }}
  {{- end }}
